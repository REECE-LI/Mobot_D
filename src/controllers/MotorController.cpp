...
_velPID(0, 0, 0,  [this]() { return this->_motor->getVelocity(); }, [this](double output) { this->_motor->run(output);}),\
_posPID(0, 0, 0,  [this]() { return this->_motor->getPosition(); }, [this](double output) { this-> _velPID.setTarget(output);})
{
...
void MotorController::attachMotorDriver(MotorDriver *motor)
{
    _motor = motor;
}
 ...
 void MotorController::init(void)
 {
    ...
    xTaskCreate(pidCalculationTask, "PID_Calculation", 4096, this, 1, &_pidCalculationTaskHandle);
 }
...
int MotorController::getVelocity(void)
{
    return  _motor->getVelocity();
}


int MotorController::getPosition(void)
{
