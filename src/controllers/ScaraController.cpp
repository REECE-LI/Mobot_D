...
ScaraController::ScaraController(ServoController *servo, DendoStepper *stepper,
                                 float l1, float l2) {
    _servo = servo;
    _stepper = stepper;
    _L1 = l1;
...
ScaraController::ScaraController(ServoController *servo, DendoStepper *stepper,
                                 TMC2300 *tmc2300, float l1, float l2) {
    _servo = servo;
    _stepper = stepper;
    _tmc2300 = tmc2300;
...
bool ScaraController::moveZ(float z) {
    ...
    if (_stepper) {
        ...
        // Serial1.println("detect pin");
        while (_stepper->getState() != IDLE) {
            //isArrived();
            Serial.printf("_limitPin: %d %d\r\n", digitalRead(_limitPin1),
                          digitalRead(_limitPin2));
    ...
    if (_tmc2300) {
        ...
        if (_z < -10000) {
            if (digitalRead(_limitPin2)) {
                _tmc2300->setVelocity(-50000);
            } else {
                // Serial.printf("1:_limitPin1: %d _limitPin2: %d
                // %f\r\n",digitalRead(_limitPin1), digitalRead(_limitPin2), _z);
                _tmc2300->setVelocity(0);
                return true;
            }
...
void ScaraController::setHomeZ() { _stepper->setHome(); }
...
bool ScaraController::getCoordinate() {
    ...
    theta1 =
            (_theta1Base - (float) (_servo1BaseAngle - pos1) / 11.38) * (M_PI / 180);
    theta2 = (_theta2Base -
              (pos1 - pos2 - (_servo1BaseAngle - _servo2BaseAngle)) / 11.38) *
             (M_PI / 180);
...
bool ScaraController::getCoordinate(float *x, float *y, float *z) {
    ...
    theta2 = (_theta2Base - (pos1 - pos2 - (_servo1BaseAngle - _servo2BaseAngle)) / 11.38) *
             (M_PI / 180);
...
bool ScaraController::inverseKinematics(float x, float y) {
    ...
    if (1) {
        ...
        pos1 = _servo1BaseAngle - (int) (((float) _theta1Base - theta1) * 11.38 + 0.5);
        pos2 = _servo2BaseAngle - (int) (((float) _theta2Base - theta2) * 11.38 + 0.5) -
               (int) (((float) _theta1Base - theta1) * 11.38 + 0.5); //+ (pos1 - lastPos1);
        ...
        // Serial1.printf("theta1: %f theta2: %f, pos1: %d pos2: %d\r\n",
        //                theta1, theta2, pos1, pos2);
        last_theta1 = theta1;
        last_theta2 = theta2;
...
#endif
bool ScaraController::linearCompensation(float x1, float y1, float x2,
                                         float y2) {
#if 1
