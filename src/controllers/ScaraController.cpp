...
bool ScaraController::moveZ(float z) {
  ...
  if (_stepper) {
    ...
    //   !digitalRead(_limitPin1), !digitalRead(_limitPin2) , _z); delay(10);
    // }
  }
...
bool ScaraController::inverseKinematics(float x, float y) {
  ...
  if (abs(cos_theta2) <= 1) {
    ...
    pos2 = _servo2BaseAngle - (int)((_theta2Base - theta2) * 11.38 + 0.5) -
           (int)((_theta1Base - theta1) * 11.38 + 0.5); //+ (pos1 - lastPos1);
