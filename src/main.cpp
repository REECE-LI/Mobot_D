...
void setup() {
    ...
    xTaskCreatePinnedToCore(SerialTask, "Serial", 1024 * 2, NULL, 3, NULL, 0);
    xTaskCreatePinnedToCore(ArmControlTask, "ArmControl", 1024 * 5, NULL, 3, NULL, 1);
    xTaskCreatePinnedToCore(StepperTask, "StepperTimer", 1024 * 2, NULL, 3, NULL, 1);
}
...
void click(void) {
    ...
    for (uint16_t i = 0; i < testGcodeSize; i++) {
        if (testGcode[i].isPen) {
            mobot.ArmMoveDown();
        } else {
            mobot.ArmMoveUp();
        }
        ...
        // 输入需要补偿的点
        Point_t inputPoint = {testGcode[i].x - 100, testGcode[i].y + 20};
        ...
        uint8_t indexX = _x / POIN_GAP;

        uint8_t indexY = _y / POIN_GAP;

        uint8_t num = indexX + (indexY) * X_NUM;
